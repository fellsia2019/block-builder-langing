...
function getBreadcrumbs(
  ...
  const docRoot = { label: t('title'), href: '/docs/get-started' };

  for (const group of DOCS_SIDEBAR) {
    for (const item of group.items) {
      if (pathname === item.href || pathname.startsWith(`${item.href}/`)) {
        if (item.href === docRoot.href) {
          return [{ label: t(item.titleKey), href: item.href }];
        }
        return [docRoot, { label: t(item.titleKey), href: item.href }];
      }
    }
  }

  if (pathname.startsWith('/docs/core/')) {
    return [docRoot, { label: t('apiReference'), href: '/docs/core/classes' }];
  }
