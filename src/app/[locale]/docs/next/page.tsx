...
export default nextConfig`}
      ...
      <section>
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4">{t.rich('serverClient.description', docRichTags)}</p>
        <CodeBlock
          language="tsx"
...
export async function POST(request: Request) {
  ...
  return NextResponse.json({ ok: true })
      ...
      <section>
        ...
        <p className="text-gray-600 dark:text-gray-400 mt-4 text-sm">{t.rich('ssrUtils.enrichNote', docRichTags)}</p>
      </section>
      ...
      <section className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-6 border border-slate-200 dark:border-slate-700">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400">
          {t.rich('liveExample.description', {
            example: (chunks) => (
